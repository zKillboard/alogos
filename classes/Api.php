...
class Api
{
		...
		public static function processApi($keyRowID, $keyID = null, $vCode = null)
		{
				...
				$pheal = Util::getPheal($keyID, $vCode);
				try {
						Db::execute("update skq_api set errorCode = 0, lastValidation = now() where keyRowID = :keyRowID", array(":keyRowID" => $keyRowID));
						$result = $pheal->accountScope->APIKeyInfo();
						$accessMask = $result->key->accessMask;
						$expires = $result->key->expires;
						if ($expires == 0) $expires = null;
						$cachedUntil = $result->cachedUntil;
						Db::execute("update skq_api set accessMask = :aM, expires = :expires, cachedUntil = :cachedUntil",
									array(":aM" => $accessMask, ":expires" => $expires, ":cachedUntil" => $cachedUntil));
